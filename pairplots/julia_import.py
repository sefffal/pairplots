...
if "juliacall" in sys.modules:
    warnings.warn(
        ...
        "Also note that pairplots.py will not be able to configure `PYTHON_JULIACALL_THREADS` or `PYTHON_JULIACALL_OPTLEVEL` for you."
    )
else:
    ...
    if os.environ.get("PYTHON_JULIACALL_THREADS", "auto") != "auto":
        warnings.warn(
            ...
            "so pairplots.py was not able to set it. You may wish to set it to `'auto'` for full use "
            "of your CPU."
        )
...
autoload_extensions = os.environ.get("PAIRPLOTS_AUTOLOAD_EXTENSIONS", "yes")
if autoload_extensions in {"yes", ""} and jl_version >= (1, 9, 0):
    ...
    warnings.warn(
        "PAIRPLOTS_AUTOLOAD_EXTENSIONS environment variable is set to something other than 'yes' or 'no' or ''."
    )

jl.seval("using PairPlots")
PairPlots = jl.PairPlots
